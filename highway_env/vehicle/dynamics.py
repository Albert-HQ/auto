...
class BicycleVehicle(Vehicle):
    ...
    def step(self, dt: float) -> None:
        ...
        # Prevent reversing: clamp to non-negative speed
        if self.speed < 0.0:
            self.speed = 0.0
        # Respect MAX_SPEED upper bound
...
def simulate(dt: float = 0.1) -> None:
    try:
        import control  # optional, only for demo plotting
    except Exception:
        print("simulate() skipped: optional 'control' package not available.")
        return
